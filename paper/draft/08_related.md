...
arXiv:2606.01839 argues that raising the scheduling unit from the turn to the conversation converts turn-level irregularity into observable structure and removes the need for prediction. It reaches a compatible conclusion from a different substrate and a different method. This paper adds *why* the prediction fails — an irreducible variance that more samples do not reduce — and *where* a lever survives without it. <!-- CLAIMS 2.8 -->

## 8.5 Scheduling and batch composition

Program-level scheduling that removes head-of-line blocking (arXiv:2502.13965) and session-centric scheduling (arXiv:2607.08565) operate above the layer studied here; the latter is the server-side coordination position that §5.3 identifies and this work did not open. Chunked prefill (OSDI '24, arXiv:2403.02310) is the direct counterpart of §3.3 and is exactly the design whose ablation in §7.2 removes the stall while increasing device time. Multi-core NPU serving (arXiv:2510.05632) studies the layer below: core placement and parallelism, held fixed throughout this work.
