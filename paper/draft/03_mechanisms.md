# ③ Three mechanisms of the return-arrival process

## 3.1 Grid alignment decides the *sign* of the gap effect
...
One asymmetry is worth stating because it makes gaps costlier than a naive account suggests: a session that returns *immediately* does not evict its own cache, because a completed block becomes evictable only after the next admission has already chosen its victim. A session that returns after a gap has lost that protection. **The cost of a gap therefore includes losing one's own shield, not merely gaining competitors.** <!-- CLAIMS 1.13 -->

**Figure ①.**

## 3.3 Prefill is charged to everyone
...
## 3.4 Why the three multiply

The mechanisms are not three independent costs to be summed. The slot pool decides **who** must recompute; exclusive prefill decides that the recomputation is charged to **everyone then decoding**; and the grid decides **how many** that is. A cache miss under a wide batch is expensive twice over.

This composition is also why an intuition that works elsewhere fails here, and §⑤ takes it up: holding a returning session back so that it can share a batch looks like it should help, and does not.
