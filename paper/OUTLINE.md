...
### 3.5 어디서 멈추는가 — "클수록 좋다"에 대한 답

- `batch_size`를 8 → 16 → 24 → 32로 밀면 **B=16에서 이득이 멈춘다** ([TASK40](../docs/research/TASK40.md), 통제된 인접쌍 2/2 `포화`).
- 기전이 두 겹으로 닫힌다: **생존율이 포화**해 살릴 캐시가 남지 않고, **최상위 눈금이 한 번도 선택되지 않아**(실측 0.0 %) 그 큰 step 비용이 부과되지도 않는다.
- 따라서 **최적 `batch_size`는 하드웨어 상한이 아니라 워크로드 분포의 함수**다 — KV 한계는 B ≈ 46(외삽)인데 이득은 그 3분의 1에서 끝난다.
- 무보정 시뮬레이터가 **"아무 일도 일어나지 않는다"를 미리 맞혔다**(N ∈ {6,8} 6칸 오차 0.0003–0.0026). 음성 예측의 확인은 양성 예측의 확인과 다른 종류의 검증이다.
- **그림 ⑨**.
