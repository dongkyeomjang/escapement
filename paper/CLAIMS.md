...
| 3.10 | **이득은 회수 가능한 캐시 손실의 잔량에 비례하고, 생존이 포화하면 끝난다** — pool을 더 키워도 살릴 캐시가 없으면 살 것이 없다 | [TASK36](../docs/research/TASK36.md), [TASK40](../docs/research/TASK40.md) | **`class`** (형태) | ⑨ | — (수치 금지). **3회 재현**: [TASK35](../docs/research/TASK35.md) N=8(잔량 큼 → +9.7 %), [TASK36](../docs/research/TASK36.md) N=6 신규 seed(잔량 없음 → 밴드 안), [TASK40](../docs/research/TASK40.md) N=10 B16(잔량 2건 → +2 %) |
| 3.11 | 이 워크로드에서 그 **포화 지점은 B=16**이다 (통제된 인접쌍 2/2, 중앙 ratio 0.9999·1.0002, CI 폭 0.0192·0.0016) | [TASK40](../docs/research/TASK40.md) | **`stack`** + 워크로드 값 | ⑨ | 눈금 `(1,4,6,8,10,B)` 고정·최상위만 변경, **N ≤ 10**, 현실 tool latency 워크로드. **N > 16은 재지 않았다** |
...
| 3.10–3.13 포화 | **높음.** "batch를 키워도 소용없다"로 일반화되기 쉽다 | **법칙(3.10)과 위치(3.11)를 다른 문장으로 낸다.** 법칙은 수치 없이("잔량이 없으면 이득이 없다"), 위치는 **N ≤ 10 · 이 워크로드**를 병기해서. 반전 구간(N > 16)을 **재지 않았다**고 같은 문단에 쓴다 |
| 3.13 KV 한계 B ≈ 46 | **높음.** 실측처럼 읽힌다 | 3점 적합의 **외삽**임을 수치와 같은 문장에 쓰고, 최상위 눈금이 넓어지면 그래프 항이 커질 수 있음을 덧붙인다 |
| 3.14 음성 예측 | 중간. "시뮬레이터가 정확하다"로 뭉뚱그려질 수 있다 | **무엇을 맞혔는지**를 적는다 — 크기가 아니라 **효과의 부재**를, 그것도 측정 전에 commit해서 |
| 1.18 계통 오차 | **높음.** "시뮬레이터가 틀렸다"로 읽히면 ④의 게이트 결과 전체가 흔들린다 | **scope 세 항을 항상 함께** 낸다 — 선등록 밴드 안, 높은 동시성 집중(확증 제외 구간), 보수적 방향. 그 다음에 "누락 항 미특정"을 적는다. **순서를 바꾸면 자기 비판이 결과 부정으로 읽힌다** |
