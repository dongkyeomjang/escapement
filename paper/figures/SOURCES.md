...
## 그림에는 내부 기록 명칭이 들어가지 않는다

**논문 산출물(렌더링 PDF, arXiv에 공개되는 TeX 소스)에는 `TASK*` 표기가 나타나지 않는다.** 그림 안에서 값의 성격을 밝힐 때는 의미론적 라벨을 쓴다.

| 라벨 | 뜻 |
|---|---|
| `measured` / 실측 | 실기기 측정값 |
| `intervention` / 개입 | 한 축만 바꾼 재compile 개입에서 나온 값 |
| `model` / `ablation` / 모형·절제 | 시뮬레이터 계산값. **측정이 아님을 그림 안에서 표시한다** |

**추적성은 이 문서가 승계한다.** 아래 표가 그림별 값 ↔ 원 TASK ↔ raw artifact 경로의 유일한 사상이며, 논문은 저장소를 인용해 그 사슬을 잇는다. `make_package.sh`가 제출 패키지에 `TASK` 문자열이 없음을 매번 검사한다.
