...
다음 권장 작업: **논문 본문 집필**. 뼈대는 [TASK37](TASK37.md)이, 정정·검수는 [TASK38](TASK38.md)이, 초록·그림 PDF는 [TASK39](TASK39.md)가 만들었다([paper/](../../paper/)). 착수를 막는 사용자 판정은 없다. **그림 PDF와 arXiv 초록은 준비돼 있으므로 남은 순서는 마크다운 본문 → LaTeX 이식이다** ([ARXIV_CHECKLIST.md](../../paper/ARXIV_CHECKLIST.md) §5). 남은 미해결 항목은 [후속 연구](#후속-연구) 절에 9개로 정리했으며 **사용자 지시 없이 착수하지 않는다.** GPU 실측은 [결정 4](#결정-4--gpua6000-교차검증-착수-시점) 개정에 따라 **조건부 이연** 상태이고 Advisor의 최소 범위 지시문 발행 전까지 GPU 서버 관련 작업을 시작하지 않는다. 측정이 포함되는 후속 작업은 선등록 후 진행한다.
