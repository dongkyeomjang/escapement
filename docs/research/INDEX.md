...
1. **예측기 차용과 online 오차 실측** — [TASK31](TASK31.md)의 headroom gate가 `PASS`이므로 진행 조건은 충족됐다. 다만 **σ\*가 현실 워크로드에서 정의되지 않으므로 최종 gate의 분모가 없다**는 사실을 함께 보고해야 한다.
2. 현실 워크로드의 headroom이 **어느 정보**에 실려 있는지 규명 — 정보 절제를 생성 길이·완료 시각 축으로 확장한다.
