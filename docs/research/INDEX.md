...
가장 최근 TASK: [TASK62](TASK62.md) — 재compile 효과와 회차 간 변동의 분리 (`DONE`)

**측정 단계 종료.** [TASK35](TASK35.md)가 이 프로그램의 마지막 본 측정이고 [TASK36](TASK36.md)이 그 미해결 확증 1건을 닫는 후속 측정이었다. **논문 조립은 [TASK37](TASK37.md)–[TASK39](TASK39.md)에서 뼈대·정정·제출 준비까지 완료**됐고 시스템 명칭은 **`Escapement`** 로 확정됐다. [TASK40](TASK40.md)이 후속 연구 3번을 해소했고 [TASK41](TASK41.md)이 **본문 초고 10절과 LaTeX 패키지**까지 만들었다. [TASK42](TASK42.md)가 첨삭·저자 확정까지, [TASK43](TASK43.md)이 repo 개명 정합화를, [TASK44](TASK44.md)가 빌드 수정 4건과 외부 검토 18건 반영·3건 기각을, [TASK45](TASK45.md)가 독자 대면 산출물에서 내부 `TASK` 표기 제거를, [TASK46](TASK46.md)이 제출 준비 최종 교정(서지 완결·초록 재작성·§V 2×2 재구성·그림 겹침 해석적 검출)을, [TASK47](TASK47.md)이 [ENVIRONMENT.md](ENVIRONMENT.md) 신설과 정합 점검을 마쳤다. **정합 점검에서 논문의 trace 출처 서술이 사실과 다름을 발견했고(F-1), [TASK48](TASK48.md)이 원 데이터셋(TraceLab v0.0.2, arXiv:2606.30560, CC BY 4.0)을 특정해 정정했다.** **[TASK54](TASK54.md)가 측정을 짧게 한 번 더 열어 [TASK23](TASK23.md) 격자 개입의 seed 한계를 제거했다** — 같은 trace를 8구성이 재사용하는 짝 비교에서 N=6의 부호 역전 소멸이 재현됐고(선등록 3/3 적중), 논문의 해당 자백 문장은 갱신할 수 있다. **[TASK55](TASK55.md)가 그 개입 격자의 `C(6)`을 실측해 보간 의존을 제거했고**(보간 오차 2.00 %가 결론을 0.7 % 이하로 움직인다), 그 과정에서 **재현성 밴드를 어느 분산 성분에서 유도해야 하는가**라는 미해결 항목을 새로 열었다. **[TASK56](TASK56.md)이 그 개입의 padding 하락을 n별로 귀속해 기전을 수치로 닫았다**(하락의 80.9 %가 n=6 step에서 오며, 몫을 정하는 것은 padding 감소폭이 아니라 step 점유율이다). **[TASK57](TASK57.md)이 그 짝 비교의 실행 의미론을 코드 수준에서 전수 확인해 '동일 trace'가 고정하는 것과 시스템 결과로 남는 것을 갈랐고, 검증되지 않은 고리 하나(재도착 prompt에 실리는 생성 text의 byte-identity)를 특정했다.** **[TASK58](TASK58.md)이 3.2절의 근거를 마감했다** — 계층 표기를 canonical로 정리하고, [TASK14](TASK14.md) 이래 열려 있던 회수 건수의 한 건 차이를 **padding용 dummy block**으로 닫았으며(배치 격자와 KV 용량이 같은 pool을 공유한다), 용량 개입 표를 논문 형식으로 확정했다. **[TASK59](TASK59.md)가 3.3절과 부록의 관측 의미를 마감했다** — 도착 간격과 prefill 지표를 소스가 지지하는 가장 좁은 이름으로 좁히고, 용량 개입에서 병행 세션 간섭이 **직접 측정 불가**임을 목록화로 확정했으며, 부록 1의 '측정 변동 수준'이라는 표현에 근거가 없음을 밝혀 수정안을 냈다. **[TASK60](TASK60.md)이 3.3의 판정 근거가 선등록이었음을 git으로 검증하고, scheduler의 '우선순위'와 '배타'를 코드 줄 단위로 분리했으며(논문은 배타만 쓰면 된다), chunked prefill 문헌의 인용 대상을 Sarathi-Serve로 특정했다.** **[TASK61](TASK61.md)이 compile 구성 탐색을 재실행해 보존되지 않았던 후보별 전표를 복원했다** — 선등록 정의로 재구성한 후보 2,077개에서 기록된 선정 `(1,4,6,8,10,16)`·batch 16이 유일 최솟값이고 평가 ratio 0.906644가 기록 0.9066과 맞으며, 전표는 결정적이다(두 process byte 동일, commit된 harness와 bit 동일). 그 과정에서 **harness의 `--max-buckets`가 문서 정의와 한 칸 어긋나** 기본값으로는 4,393개를 연다는 것을 발견했다. **[TASK62](TASK62.md)가 [TASK55](TASK55.md)의 '밴드를 어느 분산 성분에서 유도하는가'를 반복 측정으로 다뤘다** — 3 artifact × 5회차 × 4 bucket = 60 lifecycle에서 같은 artifact·같은 bucket의 회차 간 변동으로 귀무 분포를 만들고 판정했다. **R1(동일 구성 재compile A1 대 A3) 4/4 이내 → 구별되지 않음**, **R2 A1 대 A2(격자 변경)는 bucket 8에서 `OUTSIDE`(0.07 대 q95 0.06 ms, 해상도 한 단위) → 구별됨**, A2 대 A3는 4/4 이내. [TASK55](TASK55.md)의 bucket 2 차이는 이번에 0.01 ms였다. **남은 것은 첫 Overleaf 빌드와 그림 육안 검수다.** 저자·소속·감사의 글·라이선스는 확정 반영됐고, 제출은 저자 계정 작업이라 이 저장소의 범위 밖이다. 미해결 연구 항목은 아래 [후속 연구](#후속-연구) 절에 정리했다.
...
현재 주요 blocker: 없다. **미해결 사용자 결정 3건** — 셋째는 [TASK55](TASK55.md)가 연 **substrate descriptor에 실측 `C(6)`을 반영할지**다(넣는다면 `mb6` 전용 descriptor로 분리할지도 함께 정해야 한다). 앞의 둘은 다음과 같다. 하나는 [TASK50](TASK50.md)이 무처치 null 분포를 재고 보고한 **τ 하한의 자료 기반 개정 여부**다(개정한다면 기준 집계 단위를 먼저 정해야 하고, 과거 판정에 소급하면 사후 완화가 되므로 이후 측정에만 적용해야 한다). 둘째는 [TASK51](TASK51.md)이 보고한 **논문에서 실행 모델을 무엇이라 부를지**다 — 근접 용어(`static shapes`)와 차이는 특정했으나 명칭 채택은 판단 사항이다 — [결정 5](#결정-5--시스템-명칭-충돌)가 해소되어 시스템 명칭은 **`Escapement`** 다. 논문 공개 전 사용자 확인 항목(저자·라이선스·AI 고지·**그림 육안 검수**)은 [paper/ARXIV_CHECKLIST.md](../../paper/ARXIV_CHECKLIST.md) §6에 있다. Stage 2와 Track A는 blocker가 아니라 아직 실행하지 않은 상태다.
...
- **bucket 격자 정렬이 gap 효과의 부호를 결정한다** ([TASK23](TASK23.md), 개입으로 확인). `padding_slots(N) = bucket_for(N) − N`이 있으면 AGENTIC이 유리하고 0이면 불리하다. 워크로드·seed·모델·slot 수를 고정하고 격자에 bucket 6만 추가하자 N=6 pooled ratio가 1.1504 → 0.9717로 내려가 역전이 소멸했다. **[TASK54](TASK54.md)가 같은 trace를 두 격자가 재사용하는 짝 비교로 이 개입을 다시 해 부호 역전 소멸을 재현했다** (`Δp` +0.0650 → −0.0400, 반복 3/3). 다만 **이동량의 크기는 trace에 실린다**(이번 +0.1050, [TASK23](TASK23.md) +0.1418)이며 **N=8에서는 격자가 두 arm의 절대 padding만 내리고 짝 차이는 밴드 안에서 움직이지 않는다.** **법칙의 형태는 `class`, 격자·문턱·크기는 `stack`**이다. **크기 모형은 [TASK24](TASK24.md)의 시뮬레이터가 대신하며 [TASK25](TASK25.md)에서 예측력이 검증됐다** — `padding_slots(N)`은 정상 상태만 가격을 매기므로 실제 padding의 **하한**이며, 단조성이 N=4·N=5에서 깨지는 것은 batch가 N→1로 내려가는 **감쇠 경로**의 padding 때문이다.
...
| [TASK45](TASK45.md) | DONE | 논문 산출물에서 내부 TASK 표기 제거 | arXiv는 **TeX 소스를 공개**하므로 그림·소스에 내부 관리 명칭이 남으면 독자에게는 죽은 참조다. 그림 안 `TASK*`를 **의미론적 라벨**(`measured`/`intervention`/`model`)로 교체하고, ⑦의 점 옆 기록 명칭은 삭제(범례가 이미 역할을 가른다), ⑧의 x축은 **seed만** 남겼다. 편집 주석은 **이송기가 옮기지 않게** 해 경계에서 끊었고 CLAIMS id 주석 98개는 유지했다. `make_package.sh`에 청결 검사 3종(TeX·bib grep, 그림 PDF **텍스트 레이어** 추출, **패키지 ASCII 전용**)을 넣자 **지시 범위 밖의 누출 2건이 즉시 잡혔다** — 저자 소속 TODO의 한글과 CLAIMS 주석의 en dash. 지우기 전에 [SOURCES.md](../../paper/figures/SOURCES.md)의 값↔TASK↔artifact 사상이 그림 ①–⑨ 전부를 덮는지 확인해 **정보 손실 0**. 재검증 177/177, 넘침 0, 47/47. **렌더링 PDF의 `pdftotext` 검사는 TeX 미설치로 미확인** — 소스·그림 두 경로에서 0임을 확인한 간접 근거만 있다. |
| [TASK46](TASK46.md) | DONE | 제출 준비 최종 교정 | **refs.bib 전면 완결** — 19항목 저자 전체를 1차 출처로 확인(KVFlow는 NeurIPS 2025 정식 서지로 전환), 작업 메모 전부 제거. 철회 사실만 남겼다(작업 메모가 아니라 출판 상태). **초록 277 → 248단어 재작성** — when-절·어순 수리, Escapement 명시, 명령문→조건문, 7분→8분. 조건 병기는 한 글자도 줄이지 않았고 폐지 문장을 대신 뺐다. **§V를 2×2로 재구성**(중앙집중/분산 × causal/전지) — **gateway가 반환 방출권을 쥐므로 server-side scheduler의 상위 집합**이라는 논증으로 중앙집중×causal이 실증적으로 닫혔음을 명문화([CLAIMS 2.11](../../paper/CLAIMS.md) 신설). 60 %가 집계 device time이라 세션 귀속 실격 범위 밖임을 방어. **"파라미터 0개"의 정의 문단**([CLAIMS 1.19](../../paper/CLAIMS.md)), **trace 출처 문단**(665,453 레코드 / 8,058 세션 / 43 도구, 원자료 비공개 사유). **꼬리 지연 신규 보고**(재측정 없이 기존 JSONL 재분석) — p99가 p50보다 더 줄고 격차가 동시성과 함께 벌어진다(N=10에서 p99 0.72). **선등록되지 않은 사후 관측**임을 명시. **그림 겹침을 해석적으로 검출**(배치에 쓰는 폭 표로 bounding box 교차) — 8건 발견, 전부 실재, 재배치 후 0. 그림·표 라벨 12개 전건 본문 참조. Minor·Nit 전항 반영, 허위 결함 3건 기각 유지. **렌더링 PDF 검사는 컴파일 불가로 미확인.** |
| [TASK47](TASK47.md) | DONE | 논문 환경 절을 위한 실측 환경 명세 | read-only 조사로 [ENVIRONMENT.md](ENVIRONMENT.md)를 신설했다 — 하드웨어·소프트웨어·모델·관측·워크로드를 **과거 TASK 전사 없이 현물 명령 출력으로 재확인**하고 항목마다 (값·명령·출력)을 남겼다. 새로 기록된 값: Supermicro AS -4125GS-TNRT2, AMD EPYC 9254 × 2(48 core/96 thread), RAM 1,511 GiB, KMD 3.2.2, `rebel-compiler 0.11.1.post1`. artifact 7종의 크기가 전부 생성 TASK 기록과 일치하고 **`kvcache_num_blocks == batch_size`가 7종 전부에서 성립**([TASK08](TASK08.md)의 source 판정이 실물에서 확인). **정합 점검 불일치 5건** — 그중 **F-1이 중대**하다: 논문 §II-D가 trace를 "저자가 자기 사용에서 수집"이라 적었으나 [TASK31](TASK31.md)과 소스 docstring은 **"legacy가 공개 trace에서 재계산한 산출물, 원 gz 접근 불가"** 라고 기록한다. 비공개 사유 문장도 함께 무너진다. **지시대로 수정하지 않고 보고만 했으며 투고 전 정정이 필수다.** 나머지: rows/tool calls 혼동, vllm 버전 표기 불일치, `num_devices` 대 tensor parallel, 43종의 의미. `UNKNOWN` 7건. |
| [TASK48](TASK48.md) | DONE | TASK47 불일치 정정: trace 출처를 사실로 되돌림 | [TASK47](TASK47.md)이 찾은 불일치 5건을 전부 정정했다. **F-1**: legacy를 읽어 원 데이터셋을 **TraceLab**(`uw-syfi/TraceLab`, **arXiv:2606.30560**, CC BY 4.0, 사용 릴리스 **v0.0.2** = 665,453 스텝 / 743,819 도구호출 / 8,058 세션 / 52명)으로 특정하고 arXiv에서 서지를 재확인해 **인용 가능한 형태로** 다시 썼다. "저자 수집"과 프라이버시 사유 문장을 삭제하고, 실제 제약(**원 아카이브 접근 불가 — 분위수만 사용**)을 §IX 한계로 옮겼다. **arXiv 초록의 규모(약 4,300 세션)와 v0.0.2가 달라 버전을 명시**했다. 곁가지로 latency 우선순위도 반대로 적혀 있던 것을 바로잡았다(internal 우선). F-2~F-5: 스텝/도구호출/세션 구분 병기, `vllm 0.22.0+cpu` 통일, `num_devices` 해석어 제거, "채택된 43종" 조건화. CLAIMS·ENVIRONMENT 동기화 및 상호 링크로 `[NEEDS-EVIDENCE]` 0. **F-1이 어떻게 들어왔는지도 은폐 없이 기록했다** — [TASK46](TASK46.md)에서 규모는 확인하고 수집 주체는 채웠으며, 자료의 부수 특징(`claude`·`codex` 키)이 틀린 서사를 지지하는 것처럼 보였다. |
| [TASK49](TASK49.md) | DONE | 채널 일치 허용차의 사후 민감도 분석 | 채널 일치 요건의 기본값 0.02는 **관례값**이고 유도가 없다(두 번째 항 `r_BASE/B_BASE`만 [TASK36](TASK36.md)에서 유도됐다). 그 기본값을 0.005·0.01·0.02·0.03·0.05로 바꿔 게이트가 적용된 **20칸 전수**를 저장 artifact에서 다시 계산했다(측정·device 접근 없음). 채널 계산을 `config_device.py`에서 그대로 import한 덕에 **A비·B비 40개가 기록과 소수 넷째 자리까지 일치**했고, 대조 실패 시 종료하도록 걸었다. **τ 형태(`max(기본값, r_BASE/B_BASE)`)에서는 0.005–0.05 전 범위에서 뒤집히는 셀이 0개**다 — 20칸의 최소 `r/B`가 0.0356이라 0.03까지는 기본값이 허용차를 정하는 셀 자체가 없다. **고정 밴드 형태에서는 8건이 뒤집힌다** — 0.005에서 2칸이 통과→보류([TASK36](TASK36.md)·N6·TUNED 0.0063, [TASK40](TASK40.md)·N6·B32 0.0068), 0.03·0.05에서 3칸이 보류→통과. **원본 보류 5칸 중 3칸이 0.030에서 통과로 바뀐다**(차 0.0205·0.0221·0.0236, 밴드에서 0.0005–0.0036 벗어남)는 사실도 숨기지 않고 적었다. [TASK34](TASK34.md)의 나머지 2칸(0.0908·0.0700)은 0.05까지도 보류다. 지시에 따라 **평가·해석 문장 없음** — 수치는 [CHANNEL_TOLERANCE_SENSITIVITY.md](CHANNEL_TOLERANCE_SENSITIVITY.md). |
| [TASK50](TASK50.md) | DONE | 무처치 반복 측정에 의한 채널 일치 null 분포 | 허용차가 포괄해야 할 대상 — **처치가 없을 때 두 채널이 얼마나 어긋나는가** — 을 처음 쟀다. b8 구성 하나로 N ∈ {6,8} 각각 **같은 trace를 10회**(서버 매회 재기동, `SWEEP_BLOCK_ID`로 plan 고정) 돌려 45쌍 분포를 냈다. **무처치인데도 0이 아니다** — 중앙값 0.0184(N=6) / 0.0062(N=8), 95 분위 **0.0604 / 0.0320**. 선등록 분류는 N=6 **C3**(허용차가 무처치 변동보다 좁다), N=8 C2. **다만 선등록이 집계 단위를 빠뜨렸다** — 치료 셀은 3블록 합으로 비를 만드는데 선등록 짝은 1-run 대 1-run이다. 같은 단위(서로소 3-run 집계 2,100쌍 전수)로 다시 재면 95 분위가 **0.0326 / 0.0167**로 절반이 되고 분류가 C3→C2, C2→C1로 내려간다(**사후 진단, 선등록 아님**). **두 채널의 재현성이 비대칭**이다 — A′의 CV 0.0004–0.0077 대 B의 0.0099–0.0215로 N=6에서 50배. 무처치인데도 **A′가 이산적으로 변하고**(N=8에서 재사용 6/8↔5/8이 10회 중 3회 뒤집힘) 그 이산 사건이 gap을 키우지는 않았다. **선등록 예측 5건 중 1건만 적중**했고 빗나간 방향이 전부 '예측보다 넓다'인데, 예측이 3블록 합의 값을 1-run 짝의 상한처럼 쓴 탓이다. 20/20 `VALID`, 재compile 0, lifecycle 20회. **τ 개정·기존 판정 재산출 없음** — 개정 여부는 결정 항목으로 보고한다. N=6 trace의 tool gap 총량이 0.140 s(N=8은 75.976 s)라 **N 효과와 gap 총량 효과는 분리되지 않는다**(`UNKNOWN`). |
| [TASK51](TASK51.md) | PARTIAL | 실행 모델 명칭의 문헌 조사 | compile 시점에 그래프·shape·자원이 고정되는 실행 모델을 문헌이 뭐라 부르는지 조사했다. **인용 문장은 전부 1차 출처에서 직접 받았고**([TASK48](TASK48.md)의 F-1 교훈), 검색 요약에만 있는 문장은 표에 넣지 않았다. **핵심 결과: 그래프와 자원 양쪽을 하나로 묶어 부르는 확립된 명칭이 없다** — 확인된 용어가 shape 축(`static shapes`·`bucketing`·`fixed at compile time`)과 자원 축(`static pre-allocation`)으로 갈린다. **`static batching`은 쓸 수 없다** — 주류 뜻(batch 수명 동안 구성 고정, Orca의 `request-level scheduling`)으로 읽히면 사실과 반대다. 이 substrate는 compile 고정 상한 안에서 **iteration 단위로 계속 batching**하며 `[BUCKET]`의 `request_nums` 변동이 그 증거다. **근접 용어는 `static shapes`**(llm.npu, ASPLOS '25) — 차이는 우리가 그 위에 **서빙 수준 자원 배분**(`batch_size` → `kvcache_num_blocks` → 캐시 생존)까지 담는다는 것. **`PARTIAL` 사유**: 지정된 6개 학회 최근 3년에서 1차 출처로 확인한 논문이 **1편뿐**이고 나머지는 arXiv·벤더 문서·구현 소스다. keyword 검색이지 전수 조사가 아니므로 '없다'는 약한 증거다. 후보 4종(`AOT-compiled`·`compiled inference`·`graph-captured`·`compiler-scheduled`)의 학회 용례는 `UNKNOWN`. 표·정의 문장은 [EXECUTION_MODEL_TERMS.md](EXECUTION_MODEL_TERMS.md). |
| [TASK52](TASK52.md) | DONE | 기전 1 수치의 padding 비율 재계산 | 논문 3.1절이 기전 1을 **padding 비율**로 서술하기로 해서 원 문서의 **utilization 비**를 다시 만들었다. `p = 1 − u`는 조건 안의 항등식이지만 **비는 변환되지 않으므로**(`u_A/u_C`에서 `p_C − p_A`가 나오지 않는다) 조건별 `p`를 각각 산출했다 — 17셀 × 2 arm, 측정·device 접근 0. **재현 대조 48/48**(블록별 utilization 40 + pooled 비 8, 허용 5e-5)이고 대조 실패 시 수치를 내지 않게 걸었다. **개입 짝**: N=6 CONVENTIONAL의 `p`가 격자 (1,2,4,8) **0.2350** → (1,2,4,6,8) **0.0508**, `Δp` 부호가 +0.1150 → −0.0268. 격자에 6을 넣으면 **bucket 8 step이 0개**가 되어 `6→8` 행 자체가 사라진다. **서술의 '65 %'는 블록 0 단독의 `5→8`+`6→8` 합**이고, 3블록 합산은 0.529, `6→8` 단독 합산은 **0.422**([TASK23](TASK23.md) 관측 5의 값)다 — 세 값이 모두 문헌에 있다. 'agentic은 padding 0 step이 많다'는 **N ≤ 7의 8칸에서만** 성립하고 N ≥ 8의 6칸에서는 부호가 반대다. **device time '11/11, 1.05~1.78×'의 출처는 [TASK26](TASK26.md) 관측 4**이며 이번이 최초 산출이 아니다 — 11/11 재현(초 ±0.005 s). 계산 경로 2건 기록: 그 표의 `busy ratio`는 **반올림된 초의 비**(최대 차 0.00054), `token` 열은 `generated_tokens`가 아니라 **decode token = `sum_request_nums`**. 지시문의 `TASK25: N∈{3,5,7}`은 실제로 [TASK25](TASK25.md)가 {3,4,7}이고 {3,5,7}은 [TASK23](TASK23.md) 2a라 **둘 다 계산**했다. 지시에 따라 **평가 문장 없음** — 표는 [PADDING_RATIO.md](PADDING_RATIO.md). |
| [TASK62](TASK62.md) | DONE | 재compile 효과와 회차 간 변동의 분리 | 선등록 `8cd2e92`(19:12:46) → A3 compile 19:13:45 → 측정 19:22:31. **compile 1회**(A1 구성 `(1,2,4,8)`·batch 8을 같은 명령으로, `HF_HUB_OFFLINE=1` 1건 차이), **60 lifecycle 전부 유효**(재실행 1건: chunk 511). `VLLM_RBLN_METRICS`가 bucket별 구분 없는 lifetime 요약만 내므로 **회차 = bucket 4개를 각각 fresh server로 잰 한 벌**(사용자 승인). 귀무 = 같은 artifact·같은 bucket의 회차 쌍 `|Δ C_model|`, bucket별 30쌍 q95 = **0.221 / 0.141 / 0.3675 / 0.060 ms**(b1/b2/b4/b8) — [TASK55](TASK55.md) 밴드 0.03 ms보다 모두 크다. **R1 `|Δ(A1,A3)|` 0.05/0.02/0.02/0.03 → 4/4 이내, 구별되지 않음.** **R2 `|Δ(A1,A2)|` 0.02/0.01/0.02/0.07 → bucket 8 `OUTSIDE`, 구별됨**(q95 = 최댓값 0.06, 해상도 한 단위 차), `|Δ(A2,A3)|` 4/4 이내. [TASK55](TASK55.md) bucket 2의 −0.29 ms는 이번 5회차 중앙값에서 −0.01 ms. A3의 `.rbln` 5개는 A1과 크기는 같고 바이트는 전부 다르다(`mod_name` 다름, `rbln_config.json` 동일). 판정 단위(5회차 중앙값 차)의 귀무와 날짜 간 변동은 `UNKNOWN`. |
| [TASK61](TASK61.md) | DONE | compile 구성 탐색의 재실행과 provenance 복원 | **시뮬레이터 재실행**(실측·compile·serving·device 0). 대조 기준을 계산 전에 commit했다(`4cd99fd`, 첫 실행보다 16초 앞). **Q1** 선등록 정의(총 bucket ≤ 6)로 후보 **2,077개 일치** — 단 commit된 `config_search.py`는 `--max-buckets`를 중간 bucket 수 + 1로 써서 **기본값이면 4,393개**, `--max-buckets 5`면 순서까지 같은 2,077개다(원 실행 인자는 `UNKNOWN`). 기본 실행(보조, 보고만)은 7-bucket `(1,4,5,6,8,10,16)`을 고르고(평가 0.9050) 기록된 선정은 19위다 — **기록의 세 값(2,077·선정·0.9066)은 총 bucket ≤ 6 공간의 출력과만 맞는다.** **Q2** `(1,4,6,8,10,16)`·batch 16이 탐색 ratio **유일 최솟값**(0.920931, 차순위 +0.0014, 20위 +0.0045, 상위 20 전부 batch 16, batch 12 최선은 352위). **Q3** 평가 ratio **0.906644 → 0.9066 일치**, 축별 귀속 0.9279·0.9929도 일치. **Q4 결정적** — `PYTHONHASHSEED`가 다른 두 process의 전표가 byte 동일, commit된 harness 출력과 2,077개 전부 bit 동일. 원 실행이 이 인자·코드로 돌았다는 것은 원 출력이 없어 세우지 못한다. 전표는 `results/npu/stage2/20260911-153800-config-search-rerun/`(git 미추적, SHA256·상위 20표는 TASK에 수록). 논문 §4 44행 "top candidates differ by percentage points"와 이번 수치(상위 후보 간 0.14–0.45 %p)를 나란히 기록했다. |
| [TASK60](TASK60.md) | DONE | 3.3 판정 근거 확인, scheduler 소스 확인, chunked prefill 문헌 조사 | read-only 조사(**신규 측정 0**). **A**: 주입 실험([TASK22](TASK22.md))의 문턱 5.0, 동시성 기준(`max(start) < min(end)`, 공통 교집합), 대조 초과 시 `PARTIAL` 강등이 **전부 선등록에 있고** commit `ba6ee2b`(22:00:47)이 측정 시작(22:01:09)보다 **22초 앞선다**(git 검증). **B**: `optimum_scheduler.py`의 주석 한 줄이 **우선순위와 배타 두 주장**을 담는데 코드에서 둘은 다른 줄이다 — **배타는 무조건**(`:335-337` prefill 1건 제한, `:465` decode loop가 `req_index == 0`일 때만), **우선순위는 `running`에 자리가 있을 때만**(`:328-329`에서 만석이면 즉시 `break`). **논문은 배타만 쓰면 되고 그편이 더 정확하다.** **C**: "prefill이 decode를 지연시킨다"는 문장은 **Sarathi-Serve(arXiv:2403.02310, OSDI '24, 저자 8인)에만 있고 원 Sarathi(arXiv:2308.16369, 저자 6인, v1만, 게재 없음)에는 없다** — 후자의 동기는 GPU 이용률과 pipeline bubble이다. 원문 3곳을 §3.2 `Throughput-Latency Trade-off`와 §4.2 `Stall-free batching`에서 인용했고 `generation stalls` 용어 정의도 확보했다. `refs.bib`에 **원 Sarathi 항목을 신규 추가**했다(`sarathi2024`는 이미 있고 제목·저자·booktitle이 1차 출처와 일치, 쪽수는 독립 확인 실패로 그대로 둠). **D**: 9/24 대 10/24는 [TASK58](TASK58.md)에서 이미 확정 — 재작업 없이 지목만 했다(기준 구성은 반복별 3·3·3 = **9/24**, 10/24는 격자가 다른 [TASK40](TASK40.md)의 '통제되지 않은 인접쌍'). `UNKNOWN`: OSDI 게재본의 절 번호·쪽수·ISBN(USENIX·DBLP가 403). |
| [TASK59](TASK59.md) | DONE | 3.3 관측 의미와 부록 판정 기준 감사 | **신규 측정 0, compile 0, serving 0, 신규 요청 0.** 감사 계획을 재집계 전에 commit했다(`bd9b5ec`). **① 도착 간격**: `arrivals_s`는 **비어 있지 않은 SSE 조각의 client 수신 시각**이고 **조각 1개 = token 1개는 검증되지 않았다**(저장 자료에 조각별 token 수가 없음) → 논문 명칭을 **'streaming 응답 도착 간격'**으로 유지한다. 수준별 88.5/359.6/1177.2 ms와 100.9/372.6/1191.7 ms를 재현했고 **중앙값 순서(세션→반복)를 명시**했다(역순과 최대 0.03 ms 차). **'1 ms 이내 동시'는 경계 시각 일치**(시작 차 0.082–0.241 ms, 종료 차 0.101–0.432 ms)이지 교집합 폭(100–1195 ms)이 아니며, **판정 기준은 그보다 약한 '교집합 존재'** 였다 — 관측이 기준보다 강하다. 대조 3회의 큰 간격은 전부 t < 0.33 s이고 원 `PARTIAL` 판정을 보존한다. **② prefill 지표**: `first_token_ts − scheduled_ts`이며 **완료 시에만** 반영되고 **선점을 포함**한다 → 가장 좁은 이름은 **'서버 측 prefill 경과시간'**, 순수 device 실행시간이 아니다. 창 내 완료가 주입뿐임을 **count 증분 1 + bystander 0/4 종료**로 확인했다. 비용 모형 4점 재현(최대 잔차 2.4 ms), **128은 scheduler chunk 예산이지 device 분할이 아니다**(요청당 forward 1회). 기존 코드는 **`T(C)` 독립형**을 쓰고 `T(L)−T(L−C)`와 최대 27.8 ms 차가 나며, **잔여 < 500 token은 적합 구간 밖 외삽**이다. v2 보정의 ITL 합은 **서버 집계 counter**이지 client streaming 실측이 아니다. **③ 용량 개입의 간섭: 직접 측정 불가** — streaming 시계열·요청별 prefill 시각·대기 세션 수가 모두 없고 서버 로그가 1초 해상도다. `busy_s`(s)와 `stall_s`(session·s)는 별도 property로 **중복 합산 경로가 없다**. **④ 부록 1**: N=7 6블록을 `[BUCKET]` 합계에서 직접 산출(위 2/안 4/아래 0, 5/6 미충족 → `INCONCLUSIVE` 재확인), 6블록 pooled `R=1.0273`·`Δp=+0.0223`. **'측정 변동 수준'이라 쓸 직접 근거가 없어** **'사전 판정 기준에서 차이를 확정하지 못함'**으로 수정안을 냈다. **⑤ 부록 2**: 반올림 전 하락분 `0.174926…`, Shapley 잔차 **정확히 0.0**, 두 분해는 같은 자료의 다른 분해다. 산출: [EVIDENCE_3_3.md](EVIDENCE_3_3.md)(5분류 근거표)와 [PAPER_3_3.md](PAPER_3_3.md)(TASK 번호·경로 없음). [TASK22](TASK22.md) 서술 2건을 정정표에 기록했고 **원문과 판정은 그대로**다. |
| [TASK58](TASK58.md) | DONE | 3.2 근거 마감 감사: 계층 표기·회수 사건·용량 개입 | **신규 측정 0, compile 0, serving 0.** 감사 계획을 재집계보다 먼저 commit했다(`a1826b7`). **① 계층 표기**: canonical은 층 1 = inner block 128 token(LRU, 512), 층 2 = outer block 8,192 token(FIFO, 8 = `batch_size`). [TASK57](TASK57.md) §4는 **표기 오류 3곳·내용 오류 0건**으로, 번호를 바로잡으면 수식·수치·결론이 그대로 성립한다(저장소의 다른 문서는 전부 canonical). 재사용의 **성립은 층 2가, 보고되는 양은 층 1의 128 단위가** 정한다. **② 회수 한 건 차이가 닫혔다**: 회수 = `max(0, ALLOC + 1 − 8)`이 **21/21 실행에서 정확히 일치**하고, `+1`은 **decode batch가 덜 찼을 때(padding 조건) scheduler가 집는 dummy block**이다 — 그 경로는 `can_allocate`로 회수하고 `peek_dummy_block`으로 **할당 없이** 가져간다(`SCHED:594-597`, `PCM:508-529`). 로그도 일치한다(선점 0/21, 회수 블록이 끝내 재할당되지 않는 회수가 실행마다 정확히 1건). **[TASK14](TASK14.md)의 'decode가 slot을 더 요구했을 것' hypothesis는 채택되지 않았다.** m=6은 조회·재사용 **뒤** 회수, m=7은 조회 **전** 회수(`MATCHED_OB=None`)이며 m=7의 재개 요청은 방금 비워진 **같은 ID의 새 세대**를 받는다 — 세대 표기가 없으면 정반대로 읽힌다. **③ 용량 개입**(N=8, `batch_size` 8→16): 재사용 **9/24 → 24/24**, 재도착분 prefill 재계산 **22,103 → 5,079(−77.0 %)**, 추가 bucket 16은 **0 step**이나 **공통 bucket 분포는 동일하지 않다**(b1 891→936, b2 136→121; b4·b8 동일). 입력 계획은 3반복 전부 plan SHA256·`total_gap_s`·요청 길이 열이 동일하다. [TASK40](TASK40.md)의 10/24는 섞지 않았다. **두 기전이 같은 pool에서 만난다** — padding이 slot 한 칸을 먹는다. 산출: 논문용 [PAPER_3_2.md](PAPER_3_2.md)(TASK 번호·경로 없음)와 내부 [PROVENANCE_3_2.md](PROVENANCE_3_2.md). 기존 판정·수치는 하나도 바꾸지 않았다. |
| [TASK57](TASK57.md) | DONE | 세션 실행 의미론의 코드 수준 확인 | read-only 조사(측정 0). 세션은 **동시 출발하도록 설계**됐고(`ThreadPoolExecutor`, 시차·지터 코드 없음) 실제로는 `sent`가 `build_exact` 뒤에 찍혀 **12–14 ms 뒤 2–12 ms 퍼져** 출발한다. CONVENTIONAL의 '즉시 재도착'은 `sleep(0)`이 아니라 **`sleep` 호출 자체가 없고**(`gap_after_s > 0` 가드) 같은 thread에서 동기로 이어져 실측 **0.5–1.2 ms**다. **재도착 prompt는 직전 응답의 *실제 생성 text*를 이어붙인다**(`context = prompt + text`) — 계획된 합성 token이 아니다. **생성 text가 JSONL에 저장되지 않아 두 조건의 byte-identity는 저장 자료로 검증 불가**이며, 필요조건 4종은 만족한다(생성 길이 336/336, 격자 간·arm 간 `prompt_tokens` 84/84씩, 계획 회계 328/336에 −1 8건이 **네 조건 같은 자리에 재현**). 검증에는 `text_sha256` field 추가와 재측정이 필요하다(제안). **`plan_summary`가 `text_seed`·`session_id`를 담지 않으므로 [TASK54](TASK54.md) G1 해시는 길이·gap의 동일성을 증명하고 prompt 본문의 동일성은 `(base_seed, block_id)` 고정에서 구성상 따라온다** — 결론은 그대로이나 근거가 둘이다. 층 2는 [TASK24](TASK24.md) 관측 2 규칙을 새 배치에서 재확인했다 — 실제 hit **95/95**가 prefill 계산분만 세는 식과 맞고 판별력 있는 71건에서 생성분 포함 후보가 기각된다(24/95). **첫 요청 완료 시 층 2에 남는 범위에 생성분은 없다.** '동일 trace'가 고정하는 것(길이·gap·`text_seed`·sampling seed)과 시스템 결과로 남는 것(도착·완료 시각, step 열, `cached_tokens`, 생성 내용)을 전수 분리했다. |
| [TASK56](TASK56.md) | DONE | 개입의 padding 하락을 h(n)별 기여로 분해 | [TASK54](TASK54.md) 개입 배치의 CONVENTIONAL step 열에서 `p` 하락분(0.2324 → 0.0574, 0.1749)을 `actual` 요청 수 n별로 귀속했다. **새 측정 0** — 저장 `[BUCKET]` 열 계산만이다. **하락분의 80.9 %가 n=6 step(padding 2 → 0 소멸), 19.1 %가 n=5 step(padding 3 → 1 감소)에서 오고 기타 n은 0 %**다. 두 독립 분해(소멸 slot 몫 81.1/18.9, Shapley 80.9/19.1, 잔차 0)가 0.2 %p 안에서 일치한다. **몫을 가른 것은 padding 감소폭이 아니라 step 점유율**이다 — slot당 감소는 둘 다 2로 같고 step 수가 494 대 115다. 비율 분해에는 **분모 상호작용이 실재**한다: 분자 항 +0.1856(106.1 %), 분모(`Σbucket` 18.6 % 축소) 항 −0.0107(−6.1 %) — 개입은 손대지 않은 n의 padding *비율*을 오히려 올린다. **개입 후 남는 padding의 62.5 %는 `3→4` 사상**에서 오므로 다음 격자 후보의 최대 잔여 항은 bucket 3이다(산술, 측정 아님). N=8에서는 step의 57.3 %가 n ∈ {7,8}이라 개입이 닿지 않아 하락분이 0.0484로 N=6의 28 %뿐이다. **이 분해가 가능한 이유는 gap=0인 CONVENTIONAL의 step 열이 격자에 불변**이기 때문이며([TASK54](TASK54.md) 발견 5), **AGENTIC에는 같은 분해를 적용할 수 없다.** |
| [TASK55](TASK55.md) | PARTIAL | 개입 격자의 step 비용 실측과 device time 검증 | [TASK54](TASK54.md)가 남긴 세 구멍(보간 `C(6)`, 재compile 통제 미확인, padding→device time 미검증)을 [TASK13](TASK13.md)과 같은 방법으로 쟀다. 재compile 없이 **9 lifecycle, 9/9 게이트 통과**. **`C(6)` 실측 `C_fixed` = 11.660 ms로 선형 보간(11.8975)이 2.00 % 과대평가**했고, 그 오차가 device 비를 N=6에서 +0.7 %, N=8에서 +0.07 %만 움직인다 — bucket 6의 step 점유율이 12–47 %이기 때문이며, **[TASK54](TASK54.md)·[TASK52](TASK52.md)의 device time 서술은 보간 `C`로도 안전했다**. **P1(단조성)·P3(채널차 ≤ τ(N), 8/8) `PASS`**, **P2a–P2c(재compile 통제) `FAIL`**. P2 실패는 밴드가 **분산 성분을 잘못 골라서**다 — 0.03 ms는 [TASK13](TASK13.md)의 *한 session 안* level 간 범위인데, 관측된 *session 간* 변동은 같은 artifact에서도 최대 0.32 ms다. **제3의 측정([TASK13](TASK13.md)의 `mb`, 3주 전)과 삼각 대조하면 `mb6`가 네 공유 bucket 전부에서 0.04 ms 안**이고 이번 `mb`의 bucket 2만 벗어나므로 **재compile이 기존 bucket 비용을 바꿨다는 증거는 없다(사후 진단)**. 판정 밴드는 완화하지 않았다. 부수: N=6 `(1,2,4,8)`에서 `Δp` +0.065(padding은 gap 조건이 유리)인데 device 비 1.4435로 **부호가 어긋난다** — [TASK53](TASK53.md)의 반전이 개입 배치에서 재현. |
| [TASK54](TASK54.md) | DONE | 격자 개입의 동일 trace 재측정 | [TASK23](TASK23.md)의 격자 개입은 배치 사이 seed가 달라 `Δp` 이동에 격자 효과와 trace 차이가 섞여 있었다. **같은 (N, 반복) plan을 두 격자·두 arm이 전부 재사용**하게 하고 그 사실을 plan SHA256으로 검사해(G1 12/12 동일) 그 한계를 제거했다. 재compile 없이 기존 artifact 2종, 8구성 × 3반복 = **24 lifecycle, 24/24 `VALID`**. **선등록 3/3 적중** — P1: N=6에서 `Δp`가 `(1,2,4,8)` **+0.0650**(양수) → `(1,2,4,6,8)` **−0.0400**(음수)로 **부호 역전이 동일 trace에서 소멸**했고 반복 부호 3/3 일치. P2: N=8은 두 격자 모두 음수(−0.0570 / −0.0554). P3: 개입 격자 N=6의 bucket 8행 step이 **두 arm 모두 정확히 0**(`BASE`에서는 CONVENTIONAL 609 step). 무효 밴드 `X = 0.004`는 관례값이 아니라 [TASK50](TASK50.md) 무처치 반복 20칸의 `p` 재현 오차(최대 쌍 차 0.00281)에서 √2를 곱해 **측정 전에** 유도했다. **크기는 trace에 실린다** — N=6 이동량이 이번 +0.1050, [TASK23](TASK23.md) +0.1418. **N=8에서는 격자가 두 arm의 절대 padding을 크게 내리면서(`p_A` 0.148 → 0.098, `p_C` 0.091 → 0.043) 짝 차이는 −0.0016만 움직여 밴드 안**이다 — [TASK23](TASK23.md) 발견 4의 '절대값이 움직인다'와 '짝 차이가 움직인다'가 분리됐다. 부수 관측: CONVENTIONAL의 decode step 총수는 격자에 **완전히 불변**(N=6 1,288 / N=8 1,202)이나 AGENTIC은 다르다. |
| [TASK53](TASK53.md) | DONE | 논문 3.1절 표: padding 비율과 device time 병치 | [TASK52](TASK52.md)와 [TASK26](TASK26.md) 관측 4의 값만으로 3.1절 표 하나를 만들었다(새 측정·새 계산 0). 논지는 **같은 조합에서의 반전** — padding은 gap 조건이 유리한 행이 있으나 **device time은 11/11에서 gap 조건이 더 쓴다**. 표시 규칙(Δp > 0 이면서 비 > 1을 굵게)이 격자 (1,2,4,8)의 **N ≤ 7 연속 5행**을 집어낸다. **합산 단위가 행 집합을 정했다** — padding은 출처 run별로, device time은 run을 가로질러 기록돼 있어 '같은 조합'이 성립하려면 후자의 분할을 따라야 했고, 그 결과 11행 중 **7행만 [TASK52](TASK52.md)와 직접 대조 가능**하고 4행(N ∈ {3,4,7,8})은 두 run 합산이라 구성 값을 따로 남겼다. 생성기가 `padding_ratio.py`에서 **셀 정의와 합산 함수를 그대로 import**해 두 열이 같은 블록 목록·같은 코드 경로에서 나오게 했고, 대조 **18/18**(padding 7행 Δ=0.0000, device 비 11행) 실패 시 표를 쓰지 않고 종료한다. **지시문 불일치 1건**: 행을 '11개 조합 전부'라 하면서 괄호로 부연한 목록은 10개이고 N=3·5·7이 빠지고 [TASK19](TASK19.md)가 들어가 있어, 표의 논지에 따라 11칸을 채택했다. 산출: 국문 캡션 markdown + booktabs LaTeX, 출처·대조는 [TABLE_3_1_PROVENANCE.md](TABLE_3_1_PROVENANCE.md). LaTeX 판은 CJK 미지원 preamble로 컴파일되지 않음을 파일에 주석으로 명시했다. |
