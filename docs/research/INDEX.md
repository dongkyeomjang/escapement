...
다음 권장 작업: (1) **현실 워크로드의 headroom이 어느 정보에 실려 있는지 규명** — 반환 시각이 아니고([TASK31](TASK31.md)) 예측도 불가능하다([TASK32](TASK32.md)). 정보 절제를 생성 길이·완료 시각 축으로 확장하며 새 측정이 필요 없다. (2) **논문 조립**([결정 5](#결정-5--시스템-명칭-충돌)의 명칭 변경 포함). GPU 실측은 [결정 4](#결정-4--gpua6000-교차검증-착수-시점) 개정에 따라 **조건부 이연** 상태이며 Advisor의 최소 범위 지시문 발행 전까지 GPU 서버 관련 작업을 시작하지 않는다. 측정이 포함되는 작업은 선등록 후 진행한다. 사용자 지시 없이 자동 착수하지 않는다.
...
1. **현실 워크로드의 headroom이 어느 정보에 실려 있는지 규명** — 반환 시각이 아님은 [TASK31](TASK31.md)이, 그 예측 불가능성은 [TASK32](TASK32.md)가 확인했다. 정보 절제를 생성 길이·완료 시각 축으로 확장한다. **새 측정이 필요 없다.**
2. 도구 **인자**를 조건으로 삼는 예측 가능성 — 현재 trace 산출물에 인자가 없어 새 자료가 필요하다.
