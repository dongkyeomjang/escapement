...
다음 권장 작업: (1) **compile-time 구성 정책** — [TASK33](TASK33.md)의 사전 정의 분기가 지목한 유일한 회수 경로다. (2) **논문 조립**([결정 5](#결정-5--시스템-명칭-충돌)의 명칭 변경 포함). GPU 실측은 [결정 4](#결정-4--gpua6000-교차검증-착수-시점) 개정에 따라 **조건부 이연** 상태이며 Advisor의 최소 범위 지시문 발행 전까지 GPU 서버 관련 작업을 시작하지 않는다. 측정이 포함되는 작업은 선등록 후 진행한다. 사용자 지시 없이 자동 착수하지 않는다.
...
1. **compile-time 구성 정책** — [TASK33](TASK33.md)의 분기가 지목한 유일한 회수 경로다.
