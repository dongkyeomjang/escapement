...
다음 권장 작업: **Advisor 첨삭과 첫 Overleaf 빌드**. 본문 초고 10절은 [paper/draft/](../../paper/draft/)에, LaTeX 소스는 [paper/latex/](../../paper/latex/)에 있다([TASK41](TASK41.md)). **이 host에 TeX가 없어 한 번도 컴파일되지 않았으므로 첫 빌드가 첫 검증이다.** `[NEEDS-EVIDENCE]` 3건은 [NEEDS_EVIDENCE.md](../../paper/draft/NEEDS_EVIDENCE.md)에 정리했고 판정은 Advisor다. **저자란·감사의 글·AI 고지는 교수 승인 전까지 placeholder이며 arXiv·TPDS 어디에도 제출하지 않는다.** 남은 미해결 항목은 [후속 연구](#후속-연구) 절에 정리했으며 **사용자 지시 없이 착수하지 않는다.** GPU 실측은 [결정 4](#결정-4--gpua6000-교차검증-착수-시점) 개정에 따라 **조건부 이연** 상태다. 측정이 포함되는 후속 작업은 선등록 후 진행한다.
