...
- **compile 구성 선택이 device time을 7.4 % 회수하며, 지배 인자는 bucket 격자가 아니라 `batch_size`(= KV pool 크기)다** ([TASK34](TASK34.md), 실기기 관측·확증 판정은 보류). `batch_size`가 `kvcache_num_blocks`를 정하고([TASK08](TASK08.md)) 그것이 캐시 생존을, 캐시 생존이 prefill 재계산을 정한다. sim 귀속에서 batch만 +7.21 %, bucket 정제만 +0.71 %다. **구성 선택은 gap 분포에 둔감하고 동시 세션 수 상한에만 반응하므로, 재구성 판단에 다시 재야 할 통계는 그것 하나다.**
...
다음 권장 작업: (1) **[TASK34](TASK34.md)의 확증 판정 재측정** — 채널 A를 `decode + prefill`로 고쳐 선등록하고 다시 잰다. 재compile이 필요 없다. (2) **논문 조립**([결정 5](#결정-5--시스템-명칭-충돌)의 명칭 변경 포함). GPU 실측은 [결정 4](#결정-4--gpua6000-교차검증-착수-시점) 개정에 따라 **조건부 이연** 상태이며 Advisor의 최소 범위 지시문 발행 전까지 GPU 서버 관련 작업을 시작하지 않는다. 측정이 포함되는 작업은 선등록 후 진행한다. 사용자 지시 없이 자동 착수하지 않는다.
...
- TASK34에서 그 유일 경로를 실제로 열어 device time 7.4 %를 회수했다 — 정책 연구의 세 배치가 음성으로 끝난 뒤 처음 나온 양성 결과다.
...
1. **[TASK34](TASK34.md)의 확증 판정을 되살리는 재측정** — 채널 A를 `decode + prefill`로 고쳐 선등록하고 다시 잰다. **재compile이 필요 없다.**
2. `batch_size`를 16보다 키운 구성 — 재compile 예산 2회가 남아 있다.
