...
- **반환 시각 하나만 알아도 headroom의 대부분이 회수된다 — 단 예산이 충분할 때만이다** ([TASK30](TASK30.md), 계산). ε=5 s에서 omniscient의 86–88 %(N=6·8), ε=1 s에서는 6.7 %뿐이다. **정확도 문턱 σ\* = 0.74–1.27 × gap 표준편차**이므로 완벽한 예측이 필요 없고, **예산과 정확도가 서로를 대신한다.** σ\*는 gap 분포의 함수이므로 워크로드를 바꾸면 다시 계산해야 한다.
...
다음 권장 작업: (1) **현실 tool latency 워크로드로 전환하고 headroom·σ\*를 재계산** — [TASK30](TASK30.md)의 σ\*는 합성 gap의 값이므로 예측기 판정의 분모가 되지 못한다. (2) 예측기 차용과 online 오차 실측. (3) **논문 조립**([결정 5](#결정-5--시스템-명칭-충돌)의 명칭 변경 포함). GPU 실측은 [결정 4](#결정-4--gpua6000-교차검증-착수-시점) 개정에 따라 **조건부 이연** 상태이며 Advisor의 최소 범위 지시문 발행 전까지 GPU 서버 관련 작업을 시작하지 않는다. 측정이 포함되는 작업은 선등록 후 진행한다. 사용자 지시 없이 자동 착수하지 않는다.
...
1. **현실 tool latency 워크로드로 전환하고 headroom·σ\*를 재계산** — legacy TraceLab 통계(665k rows, 8,058 sessions)가 있다. [TASK30](TASK30.md)의 σ\*는 합성 `uniform:1:5` gap의 값이라 예측기 판정의 분모로 쓸 수 없다.
2. 예측기 차용과 online 오차 실측 — 위 σ\*가 판정의 분모다.
