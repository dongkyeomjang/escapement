...
가장 최근 TASK: [TASK50](TASK50.md) — 무처치 반복 측정에 의한 채널 일치 null 분포 (`DONE`)
...
현재 주요 blocker: 없다. **미해결 사용자 결정 1건** — [TASK50](TASK50.md)이 무처치 null 분포를 재고 보고한 **τ 하한의 자료 기반 개정 여부**다(개정한다면 기준 집계 단위를 먼저 정해야 하고, 과거 판정에 소급하면 사후 완화가 되므로 이후 측정에만 적용해야 한다) — [결정 5](#결정-5--시스템-명칭-충돌)가 해소되어 시스템 명칭은 **`Escapement`** 다. 논문 공개 전 사용자 확인 항목(저자·라이선스·AI 고지·**그림 육안 검수**)은 [paper/ARXIV_CHECKLIST.md](../../paper/ARXIV_CHECKLIST.md) §6에 있다. Stage 2와 Track A는 blocker가 아니라 아직 실행하지 않은 상태다.
